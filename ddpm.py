...
class Diffusion:
    ...
    def get_betas(self, schedule='linear'):
        if schedule == 'linear':
            return torch.linspace(self.beta_start, self.beta_end, steps = self.T) # HINT: use torch.linspace to create a linear schedule from beta_start to beta_end
        # add your own (e.g. cosine)
        if schedule == 'cosine':
            betas = []
            for i in range(self.T):
                betas.append(self.beta_start + 0.5 * (self.beta_end - self.beta_start) * (1 + math.cos(math.pi * i / self.T)))
    ...
    def q_sample(self, x, t):
        ...
        sqrt_alpha_bar =  torch.sqrt(self.alphas_bar[t])[:, None, None, None] # HINT: use torch.sqrt to calculate the sqrt of alphas_bar at timestep t
        #sqrt_alpha_bar = sqrt_alpha_bar[:, None, None, None] # match image dimensions

        sqrt_one_minus_alpha_bar = torch.sqrt(1. - self.alphas_bar[t])[:, None, None, None] # HINT: calculate the sqrt of 1 - alphas_bar at time step t
    ...
    def p_mean_std(self, model, x_t, t):
        ...
        mean = (x_t - beta * predicted_noise / torch.sqrt(1-alpha_bar)) / torch.sqrt(alpha) # HINT: calculate the mean of the distribution p(x_{t-1} | x_t). See Eq. 11 in the ddpm paper at page 4
        std = torch.sqrt(beta)
    ...
    def p_sample(self, model, x_t, t):
        ...
        noise = torch.randn_like(mean) #if t.item() > 1 else torch.zeros_like(mean) # HINT: sample noise from a normal distribution. It should match the shape of mean
